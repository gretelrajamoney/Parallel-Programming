...
main( int argc, char *argv[ ] )
{
	...
	if( me == BOSS )
	{
		for( int dst = 0; dst < NumCpus; dst++ )
		{
			...
			// REFERENCE: syntax taken from Slide 20 of Message Passing Interface (MPI)
			// SYNTAX: MPI_Send( array, numToSend, type, dst, tag, MPI_COMM_WORLD );
			MPI_Send( &BigSignal[dst*PPSize], PPSize + MAXSHIFTS, MPI_FLOAT, dst, 0, MPI_COMM_WORLD );
		}
	}
	else
	{	
		// REFERENCE: syntax taken from Slide 25 of Message Passing Interface (MPI)
		// SYNTAX: MPI_Recv( array, maxCanReceive, type, src, tag, MPI_COMM_WORLD, &status );
	...
	if( me == BOSS )
	{
		...
		for( int src = 0; src < NumCpus; src++ )
		{
			...
			// REFERENCE: syntax taken from Slide 21 of Message Passing Interface (MPI)
			// SYNTAX: MPI_Recv( array, maxCanReceive, type, src, tag, MPI_COMM_WORLD, &status );
